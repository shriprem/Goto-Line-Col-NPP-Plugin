...
#define PREFS_TIP_AF_ONFOCUS        L"When the Goto panel regains focus, the line and column fields in it will be refreshed with the current document's current cursor position.\r\n\r\nEffective only when panel's title bar or a blank area or the Preferences button is clicked first. This refresh is suppressed when the first click is within the line or column input fields or on the Goto button.\r\n\r\nUncheck this box if your workflow needs the Goto panel's line and column values to stay fixed regardless of the changes in current document's cursor position."
#define PREFS_TIP_AF_ONTABCHANGE    L"When a different document becomes active either via a tab switch or file open or close, the line and column fields in the Goto panel will be refreshed with the newly active document's current cursor position.\r\n\r\nUncheck this box if your workflow needs the Goto panel's line and column values to stay fixed between tab switches in Notepad++ to a different document."
...
#define PREFS_TIP_CENTER_CURSOR     L"When this box is checked, the cursor will try to position as close as possible to the center of the editor view area.\r\n\r\nIf the destination position is already within the prior view area, there will likely be no centering."
