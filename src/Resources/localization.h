...
#define PREFS_TIP_SHOW_CALLTIP      L"Uncheck this option if you do not wish to view a calltip listing the line, column and hex code of the character at the destination position.\r\n\r\n[The calltip will disappear when the document cursor position shifts.]"
...
#define PREFS_TIP_EDGE_BUFFER       L"[Effective only when Centered Cursor option is unchecked]\r\n\r\nWhen the destination character shows up with the cursor stuck at either the left or right edge of the Notepad++ document view area, it will be hard to spot the cursor and the character. Hence a buffer margin near the edges, when possible, is desirable.\r\n\nSpecify the number of characters to have as a buffer margin from either of the edges for the character at the destination position."
